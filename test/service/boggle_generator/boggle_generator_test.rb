...
class BoggleGeneratorTest < ActiveSupport::TestCase
  test "check if the generated boggle items have 4x4 dimension" do
    dim = 4
    boggle_items = BoggleGenerator.new.generate_boogle_board_random
  ...
  test "check if the generated boggle items have custom dimension" do
    dim = 6
    boggle_items = BoggleGenerator.new.generate_boogle_board_random(dim)
  ...
  test "check if the generated boggle items via the hasbro dice have 4x4 dimension" do
    dim = 4
    boggle_items = BoggleGenerator.new.generate_boggle_board_hasbro
    assert_equal dim, boggle_items.length # check row
    assert_equal dim, boggle_items[0].length # check col
  end
  ...
  test "check if generated boggle items is random" do
    ...
    boggle_items_1 = boggle_generator.generate_boggle_board_hasbro.map{|row| row.join('')}
    boggle_items_2 = boggle_generator.generate_boggle_board_hasbro.map{|row| row.join('')}
